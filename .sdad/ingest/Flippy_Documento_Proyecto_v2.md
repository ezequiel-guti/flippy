...
Versión
2.0 — Arquitectura Frontend/Backend separada
Fecha
Junio 2026
Presupuesto total
...
cada hito
3. Arquitectura técnica
La arquitectura está desacoplada en dos servicios independientes: un frontend Next.js
dedicado exclusivamente a la interfaz, y un backend FastAPI (Python) que concentra toda la
lógica de negocio. Ambos servicios se despliegan en Railway dentro del mismo proyecto,
compartiendo dashboard y gestión de variables de entorno.
3.1 Diagrama de capas
Capa Servicio Responsabilidad
Cliente (PWA) Next.js 14 (App Router) en UI, service worker, manifest, instalación
Railway iOS/Android
Backend (API Core) FastAPI (Python 3.11+) en Auth, chat RAG, ingesta de documentos,
Railway webhooks MP, streaming SSE
...
semántica — sin servicio externo
Storage Supabase Storage Archivos originales (PDF, Word,
imágenes, texto)
IA — chat Anthropic Claude 3.5 Sonnet Generación de respuestas RAG + análisis
de imágenes
...
transaccionales
Hosting Railway Dos servicios en un mismo proyecto,
deploy continuo desde GitHub, SSL

| Capa | Servicio | Responsabilidad |
| ---- | -------- | --------------- |
automático
3.2 Comunicación frontend–backend
El frontend consume el backend exclusivamente vía fetch nativo del navegador, sin librerías
externas (sin axios). Todas las llamadas están centralizadas en un único archivo de servicio
del lado del cliente.
| Elemento      | Detalle                               |     |
| ------------- | ------------------------------------- | --- |
| URL base      | https://api-flippy.botizar.com/api/v1 |     |
| Autenticación | Authorization: Bearer <JWT>           |     |
Sesión Login devuelve access + refresh token, guardados en localStorage
Streaming Server-Sent Events (SSE) leído vía ReadableStream del body de fetch
Multi-tenancy No aplica — un solo cliente, sin headers de tenant ni API key estática
3.3 Flujo de una consulta
El ciclo de vida de una consulta de usuario sigue estos pasos en orden:
1. El usuario escribe una consulta (texto) o adjunta una imagen en la PWA (Next.js)
2. El frontend envía la solicitud al backend FastAPI vía fetch con SSE habilitado
3. FastAPI genera un embedding de la consulta usando OpenAI text-embedding-3-small
4. pgvector (en Supabase) realiza búsqueda de similitud coseno y devuelve los N chunks
más relevantes del corpus
5. FastAPI construye el prompt: contexto RAG + historial de la conversación + consulta
del usuario (+ imagen si aplica)
6. Claude 3.5 Sonnet procesa el prompt y devuelve la respuesta en streaming hacia
FastAPI
7. FastAPI retransmite el streaming al frontend vía SSE y guarda la interacción en
...
Cuando el administrador sube un archivo al panel, FastAPI lo guarda en Supabase Storage,
responde de inmediato al cliente, y dispara el procesamiento como tarea en segundo plano
(background task):
• PDF: extracción de texto con pdfplumber, preservando estructura de tablas y saltos
lógicos; chunks de ~500 tokens con 50 tokens de overlap
• Word (.docx): conversión a texto con python-docx, mismo proceso de chunking
...
para búsqueda eficiente. No se utiliza una base de datos vectorial externa: a un volumen de
250 MB de corpus inicial, pgvector cubre la performance necesaria sin sumar otro servicio a
mantener.
5.2 Estrategia de chunking
Parámetro Valor Justificación
Tamaño del chunk 500 tokens Equilibrio entre contexto y precisión de
recuperación, con corte inteligente por párrafo
...
Índice pgvector IVFFlat Adecuado para corpus de 250 MB, sin necesidad
de servicio externo
Orquestación LangChain Reutiliza patrones ya probados en producción
(Python)
...
Hito 1 — Base Entorno Next.js y FastAPI desplegados 2 USD 950 (25%)
Frontend + como servicios separados en Railway, semanas
Backend estructura PWA (service worker, manifest,
íconos iOS/Android), interfaz de chat
maquetada según prototipo, sidebar de
historial, aprovisionamiento Supabase +
pgvector
Hito 2 — RAG + Panel de administración de documentos, 3 USD 1.630 (38%)
Admin + pipeline de ingesta en Python semanas
Multimodal (PDF/Word/texto), embeddings y búsqueda
vectorial en pgvector, integración Claude
para respuestas ancladas con streaming
SSE, módulo de análisis de imágenes del
usuario

| Hito | Alcance | Tiempo | Monto |
| ---- | ------- | ------ | ----- |
Hito 3 —  Sistema de autenticación email/password  1 semana USD 1.075 (25%)
| Usuarios +  | (JWT), control de planes y permisos,   |     |     |
| ----------- | -------------------------------------- | --- | --- |
| Pagos       | integración completa de suscripciones  |     |     |
recurrentes MP, lógica de webhooks en
FastAPI, estados de mora y baja
...
8. Stack tecnológico
Arquitectura desacoplada: frontend y backend como servicios independientes, desplegados
juntos en el mismo proyecto de Railway.
...
| Streaming        | Server-Sent Events (SSE) | Nativo de FastAPI             |     |
| Pagos            | Mercado Pago             | API suscripciones recurrentes |     |
Control de versiones GitHub 2 repositorios privados del cliente
(web + api)
...
Railway (backend) USD 5–15 Servicio FastAPI, según tráfico y uso de
CPU
...
Total infraestructura base USD 15–55 / mes Sin costos de IA variables; sin servicios
...
al cierre
Flippy — Documento de Proyecto v2.0 · Ezequiel Gutiérrez / Botizar · Junio 2026