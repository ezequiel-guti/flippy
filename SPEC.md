...
- **Modelo:** `claude-sonnet-5` — **desviación de este documento**, que especifica "Claude 3.5 Sonnet" (`claude-3-5-sonnet-20241022`, oct-2024). Ese id no fue verificado contra la API real (sin `ANTHROPIC_API_KEY` configurada, ver §12/DECISIONS.md) pero es casi seguro que está retirado a esta altura — mismo problema que `gemini-2.0-flash` en el Incremento 7. Se usa el modelo vigente del lineup actual como mejor estimación disponible; **requiere confirmación en vivo apenas el cliente complete el requisito de cuenta Anthropic con billing**
