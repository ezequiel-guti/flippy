...
| 22 | 2026-08-06 | §3 (F-02) | UX: `.messages` (`ChatWindow.module.css`) ancla la conversación abajo (`justify-content: flex-end`) en vez de apilarla desde arriba — en conversaciones cortas los mensajes quedan pegados al input, sin dejar hueco vacío debajo | El desarrollador siguió reportando "esa parte sin nada" abajo después del fix del delta #21 (que resolvía la burbuja vacía, un bug distinto). Inspección del DOM vía DevTools (compartida por el desarrollador) confirmó que `<html>` mide exactamente el alto del viewport — el layout ocupa el 100% correctamente, no hay overflow roto; el "hueco" era el espacio sobrante de `.messages` (flex-column sin `justify-content`) apilando los mensajes desde arriba en vez de anclarlos abajo como WhatsApp/iMessage |
| 23 | 2026-08-06 | §3 (F-02), §7 | Fix — respuestas de Flippy renderizadas como Markdown real (negrita, listas, encabezados, tablas) en vez de texto plano con los símbolos `**`/`###`/`---` literales. Nueva dependencia de producción `react-markdown` + `remark-gfm`; `next.config.mjs` declara `transpilePackages` con el árbol completo de dependencias ESM de esas librerías (requerido para que Jest pueda parsear `import`/`export` de esos paquetes) | El desarrollador compartió una captura mostrando las respuestas del chat con la sintaxis Markdown sin procesar — Gemini ya devuelve Markdown (el prompt no lo pide explícitamente pero es el formato por defecto del modelo), pero `ChatMessage` solo renderizaba `<p>{content}</p>` como texto plano. Se descartó pedirle al modelo que devuelva HTML directamente (renderizarlo con `dangerouslySetInnerHTML` sería riesgo de XSS si el corpus/usuario logra colar HTML en el contexto, y los modelos son menos consistentes generando HTML limpio que Markdown) — `react-markdown` parsea el Markdown a elementos React reales sin usar `dangerouslySetInnerHTML` |
| 25 | 2026-08-07 | §3 (F-02, F-03) | Fix (completa el delta #24, que resultó incompleto) — `min-height: 0` + `overflow: hidden` en `.sidebarPane`/`.mainPane` y `overflow: hidden` + `100dvh` en `.layout` (`app/chat/page.module.css`); `.messages` ancla la conversación abajo con `margin-top: auto` en el primer hijo en vez de `justify-content: flex-end` | El desarrollador reportó que las cabeceras seguían sin quedar fijas y que había **dos scrollbars** — evidencia de que el documento entero scrolleaba, no solo los mensajes. El delta #24 puso `overflow: hidden` en `.sidebar`/`.window` pero no en sus contenedores padre: `.sidebarPane`/`.mainPane` son **grid items** de `.layout`, y como los flex items tienen `min-height: auto` implícito — no se achican bajo su contenido, así que se desbordaban fuera del `height: 100vh` y generaban el scroll del documento. Se descartó `IOSInstallBanner` como causa (devuelve `null` en desktop). De paso se reemplazó `justify-content: flex-end` (delta #22) por `margin-top: auto` en el primer hijo: logra el mismo anclaje abajo sin el bug conocido de dejar inalcanzable el tope de una conversación larga que desborda |
| 24 | 2026-08-07 | §3 (F-02, F-03) | Fix — header del chat, buscador/botón "Nuevo chat" del sidebar y la barra de mensaje quedan fijos (estilo WhatsApp): `min-height: 0` agregado a `.groups` (`ChatSidebar.module.css`) y `.messages` (`ChatWindow.module.css`), `overflow: hidden` en los contenedores padre (`.sidebar`, `.window`) | El desarrollador marcó con recuadros rojos las tres zonas (header del sidebar + buscador, `ChatHeader`, barra de input) pidiendo que queden fijas. Causa raíz: bug clásico de flexbox — `.groups`/`.messages` tenían `flex: 1; overflow-y: auto` pero sin `min-height: 0` un hijo flex no se achica más allá del tamaño de su contenido, así que en vez de scrollear internamente empujaban el alto total del contenedor padre, arrastrando el header/buscador/footer del sidebar y el header/input del chat con el scroll general |
...
| Incremento 24 | Fix — conversación anclada abajo en vez de dejar hueco vacío (F-02) | claude-sonnet-5 | 2026-08-06 | El desarrollador siguió reportando el mismo "hueco vacío" abajo del chat después del Incremento 23 (que arreglaba un bug distinto, la burbuja vacía). Pidió inspeccionar el elemento por DevTools en vez de seguir adivinando: la captura mostró `<html>` con el alto exacto del viewport (sin overflow roto, el layout `height: 100vh` funciona bien). Causa real: `.messages` en `ChatWindow.module.css` es un contenedor flex-column sin `justify-content`, así que apila los mensajes desde arriba — en conversaciones cortas queda todo el espacio sobrante visible debajo del último mensaje, en vez de pegado al input como en WhatsApp/iMessage. Fix de una línea: `justify-content: flex-end` en `.messages` — con muchos mensajes el scroll sigue funcionando igual (patrón CSS estándar de columna invertida). 7/7 tests de `ChatWindow` sin cambios (CSS puro, sin lógica), build de producción limpio |
| Incremento 27 | Fix — scroll del documento eliminado; cabeceras e input realmente fijos (F-02, F-03) | claude-opus-5 | 2026-08-07 | Completa el Incremento 26, que resultó incompleto: el desarrollador reportó que las cabeceras seguían moviéndose y que se veían **dos scrollbars** — el del documento y el interno. Causa raíz que el incremento anterior no cubrió: el fix puso `overflow: hidden` en `.sidebar`/`.window`, pero los que desbordaban eran sus contenedores padre. `.sidebarPane` y `.mainPane` son grid items de `.layout`, y los grid items tienen el mismo `min-height: auto` implícito que los flex items — no se achican bajo su contenido, así que se desbordaban fuera del `height: 100vh` del grid y hacían scrollear el documento entero. Fix: `min-height: 0` + `overflow: hidden` en ambos panes, `overflow: hidden` en `.layout`, y `100dvh` (además de `100vh` como fallback) para que en mobile el chrome colapsable del navegador no empuje la barra de input fuera de la pantalla. Descartado `IOSInstallBanner` como causa alternativa — devuelve `null` en desktop. Adicional: `justify-content: flex-end` de `.messages` (Incremento 24) reemplazado por `margin-top: auto` en el primer hijo — mismo anclaje abajo en conversaciones cortas, pero sin el bug conocido de dejar inalcanzable por scroll el tope de una conversación larga que desborda. **Verificación:** 80/80 Jest, `tsc --noEmit` y build de producción limpios, pero jsdom no calcula layout — el fix de CSS no queda cubierto por los tests automatizados y requiere confirmación visual del desarrollador |
| Incremento 26 | Fix — header, buscador y barra de input fijos en el chat (F-02, F-03) | claude-sonnet-5 | 2026-08-07 | El desarrollador marcó con recuadros rojos tres zonas de `/chat` (header del sidebar + botón "Nuevo chat" + buscador; `ChatHeader`; barra de input) pidiendo que se comporten como en WhatsApp — fijas, sin scrollear con el contenido. Causa raíz: bug clásico de flexbox en `.groups` (`ChatSidebar.module.css`) y `.messages` (`ChatWindow.module.css`) — ambos son `flex: 1; overflow-y: auto` pero sin `min-height: 0` un hijo flex no se achica más allá de su contenido (el `min-height: auto` implícito gana), así que en vez de scrollear *adentro* de esos contenedores, el contenido empujaba el alto total del padre y arrastraba todo con el scroll general (incluido el header/buscador/footer del sidebar y el header/input del chat). Fix: `min-height: 0` en ambos contenedores + `overflow: hidden` en sus padres (`.sidebar`, `.window`) como cinturón de seguridad para que solo la lista de chats y el área de mensajes scrolleen. 20/20 tests de sidebar/window/chat-page sin cambios (CSS puro), build de producción limpio |
| Incremento 25 | Renderizado de Markdown en las respuestas del chat (F-02) | claude-sonnet-5 | 2026-08-06 | El desarrollador reportó ver los símbolos de Markdown sin procesar (`**negrita**`, `###`, `---`) en las respuestas de Flippy. `ChatMessage.tsx` reemplaza `<p>{content}</p>` por `<ReactMarkdown remarkPlugins={[remarkGfm]}>` (solo para `role === "assistant"` — los mensajes del usuario siguen como texto plano) con estilos propios en `.markdown` (`ChatMessage.module.css`) para headings/listas/código/tablas consistentes con la tipografía de marca. Se evaluó pedirle HTML directo al modelo y usar `dangerouslySetInnerHTML`, descartado por riesgo de XSS y por ser menos confiable que Markdown para un LLM. **Efecto secundario resuelto:** `react-markdown`/`remark-gfm` y su árbol de dependencias (`mdast-*`, `micromark*`, `unist-*`, `unified`, `vfile*`, etc.) son ESM puro — rompía a Jest (`Unexpected token 'export'`) porque `next/jest` solo permite transformar node_modules listados en `transpilePackages` de `next.config.mjs` (confirmado leyendo el propio código fuente de `next/jest`: el comentario dice explícitamente que la config de Jest puede *agregar* patrones a `transformIgnorePatterns` pero no reemplazar el `/node_modules/` por defecto). Se declaró el árbol completo de ~85 paquetes (derivado de `package-lock.json`, excluyendo `@types/*` y paquetes CJS como `react`/`debug`/`ms`) en `transpilePackages`. 2 tests Jest nuevos (Markdown renderizado en respuestas del asistente, texto plano preservado en mensajes del usuario — 80/80 total), `tsc --noEmit` y build de producción limpios |
| Incremento 26 | Recuperación multi-intención para preguntas compuestas (F-02, SPEC_RAG.md §7) | claude-opus-5 | 2026-08-06 | El cliente comparó Flippy contra NotebookLM con el mismo corpus cargado. En una consulta de instalación eléctrica con 6 sub-preguntas enumeradas, Flippy respondió "no dispongo de la información" para todo lo técnico y solo devolvió precios y contactos. Causa raíz en el código: `classify_intent()` devuelve UNA intención (la primera que matchea en la tabla §7.2), así que "presupuesto" clasificaba la consulta entera como `costo_obra` y el contenido técnico nunca entraba en el top-k. Fix: `split_subqueries()` divide la consulta (solo ante enumeración o 2+ signos `?` — conservador por diseño, con test que fija que ninguna de las 30 queries del eval §8 se divide), se embeben todas en una sola llamada a `embed_texts`, y `search_multi()` recupera y clasifica una vez por sub-pregunta, fusionando por round-robin (no por score global, que reintroduciría el sesgo hacia costo) con tope de 12 chunks. A cada sub-pregunta se le re-adjunta el preámbulo de la consulta para no perder el dominio al embeber. `search()` queda como wrapper de `search_multi` — un solo camino de código; la ruta de visión (F-04) sigue sin dividir el caption. QA del incremento agregó `MAX_SUBQUERIES=8` (sin tope, la cantidad de consultas a la base la decidía el texto del usuario) y `zip(..., strict=True)`. 24 tests pytest nuevos (42 en `test_retrieval.py`), suite 137/137 verde contra Supabase real. Evaluación RAG §8.6 re-corrida contra el corpus real: `recall_at_k` 0.96, `mrr` 0.78, `refusal_acc` 1.00, `intent_acc` 0.77 — idénticos al baseline del Incremento 18.7, cero regresión. Pendiente: verificar con Virgilio si la fuente que NotebookLM cita como `[conversación previa]` (log de comunidad numerado, con el detalle de tomas por ambiente y secciones de cable) está cargada en el corpus de Flippy — sin eso no se puede anotar una entrada de pregunta compuesta en `queries.json` |
| Incremento 26.1 | Fix del propio Incremento 26 — el preámbulo envenenaba la clasificación de intención | claude-opus-5 | 2026-08-06 | Encontrado ejecutando la consulta real del cliente contra el corpus real, no por revisión de código: el Incremento 26 pasaba sus 42 tests y aun así no hacía lo que decía. Al re-adjuntar el preámbulo antes de clasificar, las 6 sub-preguntas heredaban `"55 m2"` — keyword de `precio_actual`, primera en la tabla §7.2 — y las 6 clasificaban con la misma intención: el split funcionaba pero la multi-intención era ficticia. Fix: `split_subqueries` devuelve `SubQuery(text, intent_text)`; `text` (preámbulo + ítem) se embebe, `intent_text` (solo el ítem) clasifica. La misma consulta ahora da `general / general / general / tecnica / costo_obra / general`. Queda corregido de paso el diagnóstico del Incremento 26: la intención que capturaba la consulta entera era `precio_actual` por "m2", no `costo_obra` por "presupuesto". `MAX_MERGED_CHUNKS` baja 12→8 por evidencia de la misma prueba: los 4 chunks de margen se llenaban de otros rubros (revoques, contrapisos, zócalos, carpintería) traídos por la sub-pregunta de materiales contra una planilla genérica — §7.4, un chunk irrelevante es peor que un chunk menos. 4 tests nuevos (46 en `test_retrieval.py`), suite 142/142 verde, evaluación RAG §8.6 re-corrida: `recall_at_k` 0.96, `mrr` 0.78, `refusal_acc` 1.00, `intent_acc` 0.77 — idénticos al baseline del Incremento 18.7. **Hallazgo de corpus, no de código:** la prueba mostró que para esta consulta el corpus solo tiene listas de precios y directorios de contactos; no existe ningún documento con la planificación técnica que NotebookLM sí responde (tomas por ambiente, secciones de cable, metros de cañería), que NotebookLM cita como `[conversación previa]`. Ninguna mejora de recuperación puede producir contenido que no está indexado |
